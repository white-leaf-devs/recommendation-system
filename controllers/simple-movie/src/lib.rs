...
impl Controller for SimpleMovieController {
    ...
    fn insert_rating(
        ...
        let query = doc! { "item_id": item_id, format!("scores.{}",user_id) : doc!{"$exists":true}};
    ...
    fn remove_rating(
        ...
        let mongo_result = collection.update_one(doc! {"item_id": item_id}, to_delete, None)?;
        ...
        match psql_result {
            Ok(deleted_rating) => Ok(deleted_rating),
            Err(e) => {
